...
from app.modules.fraud_intelligence.schemas import FraudFingerprint, SimilarityScore
...
class FraudIntelligenceEngine:
    ...
    def generate_phash(self, image_bytes: bytes) -> str:
        """Generates a perceptual hash from raw image bytes."""
        try:
            image = Image.open(io.BytesIO(image_bytes))
            # Convert to RGB to handle alpha channels safely
            if image.mode != 'RGB':
